...
#Change from prior version: Non-diabetic KD cases
...
#print(T2D_cases)
#micro
T2Dmicro_cases <- table(UKBB_AG2$T2D_status,UKBB_AG2$micro.0.0)[1,2]
#print('micro')
#print(T2Dmicro_cases)
#micro_controls
T2Dmicro_controls <- table(UKBB_AG2$T2D_status,UKBB_AG2$micro.0.0)[1,1]
#print(T2Dmicro_controls)
#CKD
T2DCKD_cases <- table(UKBB_AG2$T2D_status,UKBB_AG2$CKD_only.0.0)[1,2]
#print('CKD')
#print(T2DCKD_cases)
#CKD controls
T2DCKD_controls <- table(UKBB_AG2$T2D_status,UKBB_AG2$CKD_only.0.0)[1,1]
#print(T2DCKD_controls)
#CKD extreme
#print('CKD ex')
T2DCKDex_cases <- table(UKBB_AG2$T2D_status,UKBB_AG2$CKD_ex.0.0)[1,2]
#print(T2DCKDex_cases)
#CKD extreme controls
T2DCKDex_controls <- table(UKBB_AG2$T2D_status,UKBB_AG2$CKD_ex.0.0)[1,1]
#print(T2DCKDex_controls)
#macro cases
T2Dmacro_cases <- table(UKBB_AG2$T2D_status,UKBB_AG2$macro.0.0)[1,2]
#print('macro')
#print(T2Dmacro_cases)
#macro_controls
T2Dmacro_controls <- table(UKBB_AG2$T2D_status,UKBB_AG2$macro.0.0)[1,1]
#print(T2Dmacro_controls)
#DN cases
T2D_DN_cases <- table(UKBB_AG2$T2D_status,UKBB_AG2$DN.0.0)[1,2]
#print('DN')
#print(T2D_DN_cases)
#DN controls
T2D_DN_controls <- table(UKBB_AG2$T2D_status,UKBB_AG2$DN.0.0)[1,1]
#print(T2D_DN_controls)
#All cases 
T2D_All_cases <- table(UKBB_AG2$T2D_status,UKBB_AG2$ALL.0.0)[1,2]
#print('All')
#print(T2D_All_cases)
#All controls
T2D_All_controls <- table(UKBB_AG2$T2D_status,UKBB_AG2$ALL.0.0)[1,1]
#print(T2D_All_controls)
#print("confirmed counts w. slides")
#DN-CKD cases
T2D_DNCKD_cases <- table(UKBB_AG2$T2D_status,UKBB_AG2$DNCKD2.0.0)[1,2]
#print('DN-CKD')
#print(T2D_DNCKD_cases)
#DN-CKD controls
T2D_DNCKD_controls <- table(UKBB_AG2$T2D_status,UKBB_AG2$DNCKD2.0.0)[1,1]
    ...
    for(kk in c('Model1')){
          for (jj in c('CKD_only.0.0','CKD_ex.0.0','micro.0.0','macro.0.0','DN.0.0','ALL.0.0','DNCKD2.0.0')) {
              for (mm in c(iteration:iteration)) {
                ...
                if (jj=='CKD_only.0.0') {
                   kidney_size <- T2DCKD_cases #actually ND cases (just labeling not changed)
                   #identify sample ids for T2D cases
                   T2D_cases <- UKBB_AG2 %>% filter(T2D_status==0 & (CKD_only.0.0=='CKD'|CKD_only.0.0=='CKD controls')) %>% 
                                             select(f.eid) #20,120
                } else if (jj=='micro.0.0') {
                    kidney_size <- T2Dmicro_cases
                    T2D_cases <- UKBB_AG2 %>% filter(T2D_status==0 & (micro.0.0=='normo'|micro.0.0=='micro')) %>% 
                                             select(f.eid) #20,291
                    
                } else if (jj=='macro.0.0') {
                    kidney_size <- T2Dmacro_cases
                    T2D_cases <- UKBB_AG2 %>% filter(T2D_status==0 & (macro.0.0=='normo'|macro.0.0=='macro')) %>% 
                                             select(f.eid)
                    
                } else if (jj=='CKD_ex.0.0') {
                    kidney_size <- T2DCKDex_cases
                    T2D_cases <- UKBB_AG2 %>% filter(T2D_status==0 & (CKD_ex.0.0=='CKD controls'|CKD_ex.0.0=='CKD extreme')) %>% 
                                             select(f.eid)
                    
                } else if (jj=='DN.0.0') {
                    kidney_size <- T2D_DN_cases
                    T2D_cases <- UKBB_AG2 %>% filter(T2D_status==0 & (DN.0.0=='yes'|DN.0.0=='no')) %>% 
                                             select(f.eid)
                    
                } else if (jj=='ALL.0.0') {
                    kidney_size <- T2D_All_cases
                    T2D_cases <- UKBB_AG2 %>% filter(T2D_status==0 & (ALL.0.0=='yes'|ALL.0.0=='no')) %>% 
                                             select(f.eid)
                    
                } else if (jj=='DNCKD2.0.0') {
                    kidney_size <- T2D_DNCKD_cases
                    T2D_cases <- UKBB_AG2 %>% filter(T2D_status==0 & (DNCKD2.0.0=='DNCKD'|DNCKD2.0.0=='DNCKD Control')) %>% 
                                             select(f.eid)
                ...
                UKBB_shuffle <- UKBB_shuffle %>% mutate(jj2:=case_when(T2D_status==0 & shuffle_pheno=='cases' & jj=='CKD_only.0.0' ~ 'CKD',
                                          T2D_status==0 & shuffle_pheno=='controls' & jj=='CKD_only.0.0' ~ 'CKD controls',
                                          T2D_status==1 & jj=='CKD_only.0.0' ~ as.character(CKD_only.0.0),
                                                                      
                                          T2D_status==0 & shuffle_pheno=='cases' & jj=='micro.0.0' ~ 'micro',
                                          T2D_status==0 & shuffle_pheno=='controls' & jj=='micro.0.0' ~ 'normo',
                                          T2D_status==1 & jj=='micro.0.0' ~ as.character(micro.0.0),
                                                                      
                                          T2D_status==0 & shuffle_pheno=='cases' & jj=='CKD_ex.0.0' ~ 'CKD extreme',
                                          T2D_status==0 & shuffle_pheno=='controls' & jj=='CKD_ex.0.0' ~ 'CKD controls',
                                          T2D_status==1 & jj=='CKD_ex.0.0' ~ as.character(CKD_ex.0.0),
                                                                      
                                          T2D_status==0 & shuffle_pheno=='cases' & jj=='macro.0.0' ~ 'macro',
                                          T2D_status==0 & shuffle_pheno=='controls' & jj=='macro.0.0' ~ 'normo',
                                          T2D_status==1 & jj=='macro.0.0' ~ as.character(macro.0.0),
                                                                      
                                          T2D_status==0 & shuffle_pheno=='cases' & jj=='DN.0.0' ~ 'yes',
                                          T2D_status==0 & shuffle_pheno=='controls' & jj=='DN.0.0' ~ 'no',
                                          T2D_status==1 & jj=='DN.0.0' ~ as.character(DN.0.0),
                                                       
                                          T2D_status==0 & shuffle_pheno=='cases' & jj=='ALL.0.0' ~ 'yes',
                                          T2D_status==0 & shuffle_pheno=='controls' & jj=='ALL.0.0' ~ 'no',
                                          T2D_status==1 & jj=='ALL.0.0' ~ as.character(ALL.0.0),
                  
                                          T2D_status==0 & shuffle_pheno=='cases' & jj=='DNCKD2.0.0' ~ 'DNCKD',
                                          T2D_status==0 & shuffle_pheno=='controls' & jj=='DNCKD2.0.0' ~ 'DNCKD Control',
                                          T2D_status==1 & jj=='DNCKD2.0.0'~ as.character(DNCKD2.0.0)))
                ...
                OR_CI <- list(shuffle_type="NDcases", var=jj,var2=ii,nd_ci=TABLE2_ND,t2d_ci=TABLE2_T2D,all_ci=TABLE2,shuffle_n=mm2)
                ...
                #Total Sample Score
                  ...
                  coef_all3 <- rbind(coef_all3,coef_all2)
                    ...
                    #Export the Sheet
                    write.csv(coef_all4,paste(temp_dir,"/All_10May21_NDShuffle_it",iteration,".txt",sep=""))
                    write.csv(coef_all4_t2d,paste(temp_dir,"/T2D_10May21_NDShuffle_it",iteration,".txt",sep=""))
                    write.csv(coef_all4_nd,paste(temp_dir,"/ND_10May21_NDShuffle_it",iteration,".txt",sep=""))
